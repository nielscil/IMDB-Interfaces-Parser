...
namespace IMDB_Parser.Parsers
{
    public class MoviesParser : BaseParser
    {
        ...
        private const string _regex = @"^(?'title'.*?)\s*\(((?'year'\d{4}(\/.*)?)|(.{4}(\/.*?)?))\)\s*(?'episode'\{.*\})?\s*(\((?'videomovie'V)\))?\s*(\((?'tvmovie'TV)\))?\s*(\((?'videogame'VG)\))?\s*(\{{2}.*\}{2})?\s*(?'extra'\(.*\)[.]?)?\s*((?'period'\d{4}|.{4})(-(?'endperiod'\d{4}|.{4}))?)$";
        ...
        protected override string Header
        {
            get
            {
                return "Title; Year; PeriodBegin; PeriodEnd; Serie; Episode; VideoMovie; TVMovie; VideoGame; Extra;";
            }
        }
        ...
        private string GetMovie(string line)
        {
            ...
            if (match.Success)
            {
                movie = $"{GetTitle(match)}; {GetYear(match)}; {GetPeriodBegin(match)}; {GetPeriodEnd(match)}; {IsSerie(match)}; {GetEpisode(match)}; {IsVideoMovie(match)}; {IsTVMovie(match)}; {IsVideoMovie(match)}; {GetExtra(match)};";
            }
            else
            {
